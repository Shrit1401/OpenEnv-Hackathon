...
const chooseFallbackAction = (observation: JuryObservation | null, actions: string[]) => {
  ...
  // Add mild exploration so autoplay does not replay the same deterministic path every run.
  if (actions.length > 1 && Math.random() < 0.2) {
    return pickRandom(actions) ?? actions[0] ?? 'request_recess'
  }
  ...
  if (phase === 'voir_dire') {
    ...
    if (has('challenge_juror') && observation.remaining_challenges > 0 && Math.random() < 0.25) {
      return 'challenge_juror'
    }
  ...
  if (phase === 'witness_exam') {
    if (has('request_recess') && Math.random() < 0.2) return 'request_recess'
    if (has('call_witness')) return 'call_witness'
    if (has('request_recess')) return 'request_recess'
  }

  if (phase === 'cross_examination') {
    const hostileCount = observation.juror_moods.filter((mood) => mood === 'hostile').length
    if (hostileCount >= 5 && has('aggressive_cross') && Math.random() < 0.6) return 'aggressive_cross'
    if (has('impeach_witness') && Math.random() < 0.5) return 'impeach_witness'
  ...
  if (phase === 'closing') {
    const options = [
      has('closing_reasonable_doubt') ? 'closing_reasonable_doubt' : null,
      has('closing_procedural') ? 'closing_procedural' : null,
      has('closing_emotional') ? 'closing_emotional' : null,
    ].filter((item): item is string => item !== null)
    return pickRandom(options) ?? actions[0] ?? 'request_recess'
  }
